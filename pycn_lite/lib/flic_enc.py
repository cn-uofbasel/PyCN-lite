...
def bytesToManifest(repo, name, data):
    ...
    for r in raw:
        ...
        pycn_lite.client.cli.repo_store_chunk_bytes(repo, subname, chunk, hashId)
        ptrs.append(hashId)
    ...
    while len(tables) > 0:
        ...
        tailPtr = hashId
        pycn_lite.client.cli.repo_store_chunk_bytes(repo, n, chunk, hashId)
        start = end
    return (name, buf)
