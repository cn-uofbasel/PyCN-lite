...
class access_point():
    ...
    def fetch_pkt(self, name, digest=None):
        pkt = pycn_lite.lib.packet.InterestPacket(name, hashRestriction=digest)
        for i in range(RECV_MAX_RETRY_COUNT):
            ...
            if type(reply) == pycn_lite.lib.packet.ContentPacket:
                return reply
            if type(reply) == pycn_lite.lib.packet.NackPacket:
